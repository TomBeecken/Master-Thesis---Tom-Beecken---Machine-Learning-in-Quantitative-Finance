...
def fix_page_numbers(normalized_text, repaired_pages):
    ...
    for ind, text in normalized_text.items():
        ...
        if re.search(r'partiitem1\.business', nospace, re.IGNORECASE):
            ...
            if re.search(r"partiiitem5\.marketforregistrant|partiiitem5\.marketfortheregistrant", nospace, re.IGNORECASE):
                index = text
                next
            else:
                ...
                # Check if string contains anything other than a number
                numeric_chars = re.findall(r'\d', start_page)
                start_page = ''.join(numeric_chars)

                page_diff = int(start_index) - int(start_page)

    #adjust page numbers so they match with dictionary keys
    ...
    if page_diff != 0:
        ...
        for key in normalized_text:
            value = normalized_text[key]
            value_code = repaired_pages[key]
            new_key = key - page_diff
            if new_key >= 1:
                updated_dict[new_key] = value
                updated_dict_rep[new_key] = value_code
        
        normalized_text = updated_dict
        repaired_pages = updated_dict_rep

    return normalized_text, repaired_pages, index, page_diff
